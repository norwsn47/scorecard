...
The React Testing Library harness is in (`vitest.setup.js`, `setupFiles` in `vite.config.js`, `@testing-library/react` + `jest-dom` + `user-event`). Covered so far: `ParDelta` (§5.3 notation + colour override), the `History` player filter (#51/#61), **the edit-past-round flow (#22)** — `Scorecard.edit.test.jsx` — the `CourseEdit` screen incl. its 401 branch (#75), and (8 Sep 2026) **`Login.jsx`** (`Login.test.jsx` — both screens' "← Home" back button, the request-link POST + confirmation swap, server-error handling) and **`PageHeader`** (`PageHeader.test.jsx` — the optional-`title` skip-the-`<h1>` behaviour from #69, verbatim `backLabel`, back button only with `onBack`). Still no render coverage for:
