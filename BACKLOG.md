...
With par now shipped (#37 — `hole_pars` on the game, `scoreToPar()` helper already in `src/utils/scores.js`), show each entered score's result vs par — a small superscript `+1` / `-1` / `E` trailing the number in the scorecard grid, live as it's entered, on the read-only Summary / History scorecard, and on the share image (§4.7). **Fully specified in PRD §5.3 / §5.3.1** — notation, placement and colour all locked (3 Sep). Colour is a **semantic under / level / over set**. **Blocked on a design-director pass over DESIGN.md** to define the exact tokens, their sunlight-contrast values, and the override rule for accent (winner) / inverted (active cell) contexts. Once that lands, #38 and #52 build together on branch `feat/score-vs-par-display`. The canvas renderer (`src/utils/share.js`) needs the same colour tokens mirrored into its local constants.
...
### 64. Recolour the #39 end-of-round tally to the §5.3 semantic colour set
The #39 eagle / birdie / par / bogey tally shipped monochrome. The §5.3 display standard (3 Sep) has since locked a semantic under / level / over colour set for every vs-par figure. Once the design-director defines the tokens in DESIGN.md (the #38 / #52 build blocker), apply them to the tally cells too — a hole under par (eagle, birdie) in the "under" colour, par neutral, bogey in the "over" colour. Small `Summary.jsx` + `share.js` change; do it in the same pass as #38 / #52 if the timing lines up, otherwise straight after.
