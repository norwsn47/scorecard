...
### 69. Three back-button implementations outside `PageHeader` should compose it, not restyle it
Flagged by design-director during #44 (5 September 2026), deliberately not fixed in that pass. `src/pages/Login.jsx` (both screens, lines ~40-45 and ~68-73) and `src/pages/Summary.jsx`'s bespoke header (lines ~157-207, which reimplements the whole header shell rather than rendering `PageHeader` — presumably because Summary needs a conditional spacer / Edit-or-Done right slot `PageHeader` doesn't yet support) each hand-roll their own copy of "a button that looks like `PageHeader`'s back button." This is how the Login "← Back" leftover slipped through code review in #43b — nothing enforced one shared implementation. Fix: extend `PageHeader`'s API as needed (e.g. a richer `right` slot) and migrate Login and Summary onto it, so a future header change only has to happen once. Frontend-developer work, no design decision needed first.
