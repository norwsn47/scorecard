...
### 65. Advance button uses the wrong colour token
`Scorecard.jsx` fills the → advance button with `bg-muted border-2 border-muted` (`--color-text-muted: #6B6560`, a text-role value). DESIGN.md specifies the `control-warm` token (`--color-control-warm: #9A9189`), a warm grey created specifically for this button — currently unused. Fix: swap to `bg-control-warm border-2 border-control-warm`. Cosmetic; small. Was tracked as a "Divergence" note in DESIGN.md, moved here 4 September 2026 so DESIGN.md describes intended state only. Ties into #44.
