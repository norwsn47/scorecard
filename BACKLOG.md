...
- `Home.jsx` and `BruntsfieldCoursePage.jsx` are near-copies (header, info icon, 1/2/3 list, Last round and Sign-in links).
- Low: client email regex looser than server `EMAIL_RE`; security-notice email re-inlines the branded shell; the backend keeps its own copies of the hole cap 36 and the par band 2-7 (`functions/_lib/game-input.js`, `hole-pars.js`, the `games` handlers, `verify.js`), guarded for the par band only by `src/constants.test.js`.
