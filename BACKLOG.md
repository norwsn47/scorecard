...
- **`Login.jsx` has no render test at all**, and `PageHeader`'s optional-`title` behaviour (skips the `<h1>` entirely when falsy, added for #69) is untested. This is precisely the coverage gap that let a stale "← Back" label on Login survive #43b's review — worth prioritising over the others here if this list is ever triaged.
